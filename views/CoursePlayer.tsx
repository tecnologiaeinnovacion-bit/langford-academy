...
const CoursePlayer: React.FC = () => {
  ...
  const certificateDownloadUrl = useMemo(() => {
    if (!isCertificatePaid) return null;
    const record = getCertificateRecord(course.id);
    ...
    const studentName = user?.name || 'Estudiante Langford';
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="1200" height="850">
        ...
        <text x="600" y="360" text-anchor="middle" font-family="Georgia" font-size="54" fill="#ffffff">${studentName}</text>
                   ...
                   {certificateDownloadUrl ? (
                     <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                       ...
                       <a
                         ...
                         target="_blank"
                         rel="noreferrer"
                         className="text-xs font-black uppercase tracking-widest text-black/70"
                       >
                     ...
                     <p className="text-sm text-black/70 font-semibold">
                       Estamos generando tu certificado. Recarga la página si no aparece en unos segundos.
                     </p>
                   )}
                 </div>
               ) : (
